...
class DFOSCSpectrograph(spectrograph.Spectrograph):
    """
    Child to handle DK154 DFOSC spectrograph
    """
    ndet = 1
    name = 'dk154_dfosc'
    telescope = telescopes.DK154TelescopePar()
    camera = 'DFOSC'
    ...
    def get_detector_par(self, det, hdu=None):
        ...
        detector_dict = dict(
            binning         = binning,
            det             = 1,     # There is only one detector
            dataext         = 0,     # Data lives in HDU 0 of FITS file.
            specaxis        = 1,     # Vertical slits have horizontal wv. dispersion
            specflip        = True,  # increasing x pixel is blue -> red
            spatflip        = False, # Doesn't really matter.
            xgap            = 0.,
            ygap            = 0.,
            ysize           = 1.,
            platescale      = 0.396,
            ...
            nonlinear       = 0.86,
            datasec         = np.atleast_1d('[:,:]'),  # Unbinned
            oscansec        = None, #np.array()'[1:50,1:50]',
            numamplifiers   = 1,
            gain            = np.atleast_1d(0.16), # e-/ADU : Incorrect - update with flats
...
#        detector_dict['ronoise'] = np.atleast_1d(head1['RDNOISE'])  # e-
    ...
    def default_pypeit_par(cls):
        ...
        par['reduce']['findobj']['snr_thresh'] = 10.0  # Some CCD features will be picked out as objects if SNR is too low
        par['reduce']['findobj']['find_trim_edge'] = [900, 800] # ignore anything not in ypix 900 < y < 1100
        #par['red']
        ...
        par['calibrations']['wavelengths']['lamps'] = ['dfosc'] # Che
        ...
        par['calibrations']['standardframe']['exprng'] = [None, 450]
        ...
        for frametype in [
            "bias", "dark", "arc", "tilt", "align", "trace", "sky", "standard"
        ]:
            par['calibrations'][f'{frametype}frame']['process']['use_illumflat'] = False
            #par['calibrations'][f'{frametype}frame']['process']['use_pixelflat'] = False
    ...
    def init_meta(self):
        ...
        self.meta['decker'] = dict(ext=0, card='DFAPRNM') # Also separate by aperture.
        ...
        self.meta['dispname'] = dict(ext=0, card='DFGRINM') # Dispersion element
    ...
    def raw_header_cards(self):
        ...
        """
        return ['DFGRINM', 'DFAPRNM', 'BINX', 'BINY']
    ...
    def check_frame_type(self, ftype, fitstbl, exprng=None):
        ...
        if ftype == 'science':
            return good_exp & (fitstbl['idname'] == 'SPECSCI')
        if ftype in ['standard']:
            return good_exp & (fitstbl['idname'] == 'SPECSCI')
                               #| (fitstbl['target'] == 'STD') | (fitstbl['target'] == 'STD,SLIT'))
        if ftype == 'bias':
            return good_exp & (fitstbl['idname'] == 'BIAS')
        if ftype in ['pixelflat', 'trace', 'illumflat']: #, 'illumflat']:
            #return good_exp & (fitstbl['idname'] == 'FLAT,SKY')
            return good_exp & (fitstbl['idname'] == 'FLAT,DOME')
        if ftype in ['dark']:
    ...
    def config_specific_par(self, scifile, inp_par=None):
        ...
        elif self.get_meta_value(scifile, 'dispname') == 'G15':
            par['calibrations']['wavelengths']['reid_arxiv'] = 'wvarxiv_d154_dfosc_vert_20250401T1344.fits'
            #par['calibrations']['wavelengths']['reid_arxiv'] = 'd154_dfosc_grism15.fits'

        else:
            log.warn('dk154_dfosc.py: YOU NEED TO ADD IN THE WAVELENGTH SOLUTION FOR THIS GRISM')
